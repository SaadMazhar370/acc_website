			...
			foreach ( $customer_orders->orders as $customer_order ) {
				...
				<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $order->get_status() ); ?> order">
					...
					<?php elseif ( $is_order_number ) : ?>
						...
						<a href="<?php echo esc_url( $order->get_view_order_url() ); ?>" aria-label="<?php echo esc_attr( sprintf( __( 'View order number %s', 'edumall' ), $order->get_order_number() ) ); ?>">
						...
						if ( ! empty( $actions ) ) {
							foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
								if ( empty( $action['aria-label'] ) ) {
									...
									$action_aria_label = sprintf( __( '%1$s order number %2$s', 'edumall' ), $action['name'], $order->get_order_number() );
								} else {
