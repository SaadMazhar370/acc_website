...
$withdraw_method_key = tutor_utils()->avalue_dot( 'withdraw_method_key', $saved_account );
switch ( $withdraw_method_key ) :
	case 'bank_transfer_withdraw':
		$withdraw_method_name = __( 'Bank Transfer', 'edumall' );
		break;
	case 'paypal_withdraw':
		$withdraw_method_name = __( 'PayPal', 'edumall' );
		break;
	default:
	...
	<div class="dashboard-content-box withdraw-page-current-balance">
		...
		<div class="withdraw-balance-row">
			<p class="withdraw-balance-col">
				...
				<?php if ( $is_balance_sufficient ) : ?>
					<?php
					echo wp_kses_post( sprintf( __( 'You have %s ready to withdraw now', 'edumall' ), "<strong class='available_balance'>" . $available_for_withdraw_formatted . '</strong>' ) );
					?>
				<?php else : ?>
					<?php
					echo wp_kses_post( sprintf( __( 'You have %s and this is insufficient balance to withdraw', 'edumall' ), "<strong class='available_balance'>" . $available_for_withdraw_formatted . '</strong>' ) );
					?>
				<?php endif; ?>
			</p>
		</div>
		...
		<div class="current-withdraw-account-wrap withdrawal-preference inline-image-text">
			...
			<span>
	            ...
	            echo esc_html( $withdraw_method_name ? sprintf( __( 'The preferred payment method is selected as %s. ', 'edumall' ), $withdraw_method_name ) : '' );
	            echo wp_kses( /* translators: %1$s: a tag start, %2$s: a tag end */ sprintf( __( 'You can change your %1$s Withdraw Preference %2$s', 'edumall' ), '<a href="' . esc_url( $my_profile_url ) . '" class="link-transition-02">',
		            '</a>' ), array(
		            'a' => array(
			            'href'  => true,
			            'class' => true,
						...
						<tr>
							<td>
								...
								<div class="inline-image-text is-inline-block">
									...
									<span>
                                        <?php
                                        echo '<strong class="withdraw-method-name">' . $withdraw_method_name . '</strong>';
