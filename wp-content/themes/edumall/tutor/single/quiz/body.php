...
<div id="tutor-quiz-image-matching-choice" class="tutor-quiz-wrap tutor-quiz-wrap-<?php the_ID(); ?>">
	...
	if ( $attempt_remaining >= 0 && $is_started_quiz ) { // Check == 0 bs start quiz reduced attempt before quiz submit.
		...
		require tutor()->path . '/templates/single/quiz/parts/meta.php'; // Edumall fix.

		/* Quiz Question & Answer */
		if ( is_array( $questions ) && count( $questions ) ) {
			//require __DIR__ . '/parts/question.php';
			require tutor()->path . '/templates/single/quiz/parts/question.php'; // Edumall fix.
		} else {
			?>
			<div class="start-quiz-wrap">
				<form id="tutor-finish-quiz" method="post">
					...
					<button type="submit" class="tutor-btn" name="finish_quiz_btn" value="finish_quiz">
						<i class="icon-floppy"></i> <?php esc_html_e( 'Finish', 'edumall' ); ?>
					</button>
				</form>
			</div>
			<?php
	...
	if ( ! empty($previous_attempts) && (! $is_started_quiz || $attempt_remaining < 0) ) {
