...
if ( ! class_exists( 'Edumall_Course_Query' ) ) {
	class Edumall_Course_Query {
		...
		public function get_meta_query( $meta_query = array(), $main_query = false ) {
			...
			self::maybe_fix_meta_query_with_tutor_330( $meta_query );
		 ...
		 * @return array
		 */
		public static function get_main_meta_query( $log = false ) {
		...
		/**
		 * Since Tutor Pro ver 3.3.0 add meta query to main query that change the first relation AND to OR
		 * then this broken other meta query. We move them as sub array to fix that.
		 *
		 * @param $meta_query
		 *
		 * @return mixed
		 *
		 * @see \TUTOR_PRO\CourseComingSoon::coming_soon_meta_query()
		 */
		public static function maybe_fix_meta_query_with_tutor_330( &$meta_query ) {
			if ( ! empty( $meta_query ) ) {
				$coming_soon_query = [];

				foreach ( $meta_query as $key => $query ) {
					if ( 'relation' === $key ) {
						$meta_query[ $key ] = 'AND'; // Revert relation.
					} else {
						if ( ! empty( $query['key'] ) && $query['key'] === '_tutor_course_enable_coming_soon' ) {
							$coming_soon_query[] = $query;
				...
				if ( ! empty( $coming_soon_query ) ) {
					$coming_soon_query['relation'] = 'OR';
					$meta_query[]                  = $coming_soon_query;
				}
			}
