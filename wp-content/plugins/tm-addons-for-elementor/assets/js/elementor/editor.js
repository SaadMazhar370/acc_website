(
	function( $ ) {
		...
		function TMAddonsBuilder() {
			...
			self.init = function() {
				if ( elementorV2 || elementor.panel ) {
					self.run();
				} else {
			...
			self.addMenuConditions = function() {
				if( elementorV2?.editorAppBar?.documentOptionsMenu?.registerAction ) {
					elementorV2.editorAppBar.documentOptionsMenu.registerAction({
						id: 'tm_conditions',
						priority: 70,
						useProps: () => ({
		...
		$( window ).on('elementor:init', () => {
			const builder = new TMAddonsBuilder();
			builder.init();
		});
	}
)( jQuery );
